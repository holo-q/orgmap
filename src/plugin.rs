...
use crate::institution::{Institution, Project};
...
pub fn plugin_report(institution: &Institution) -> PluginReport {
    let mut plugins = institution
        .projects
        .iter()
        .filter(|project| project.local && !project.blacklisted)
        .flat_map(plugin_surfaces)
        .collect::<Vec<_>>();
    plugins.sort_by(|a, b| {
        a.project
            .cmp(&b.project)
...
fn plugin_surfaces(project: &Project) -> Vec<PluginSurface> {
    let Some(path) = &project.path else {
        return Vec::new();
    };
    find_plugin_manifests(path)
        .into_iter()
        .map(|(host, manifest)| plugin_surface(project, host, manifest))
        .collect()
}

fn plugin_surface(project: &Project, host: PluginHost, manifest: PathBuf) -> PluginSurface {
